...
(`map/scanners/CONTRACT.md`) and compiled into the package (Pass 8.5 +
...
`node assay.mjs enumerate <target>` and take its **CHANNEL
...
> conventions by reading a prior run. `map/validate.mjs` enforces all of it and
...
- **Counted measures come free** from the effect/capability fields the passes
 ...
 `views/improve/maturity.mjs` computes them. Improvement-loop and verification ride the
...
(`views/improve/templates/howto.md`, `concepts.md`, `method.md`, `glossary.yaml`) cost nothing per run.
...
`node assay.mjs score <run-dir> --answers <target>/ANSWERS.yaml`. It reports **recall**
