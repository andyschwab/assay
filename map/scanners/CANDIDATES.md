...
`/map/scanners/CONTRACT.md` (the peer port in §2–3,
...
- **Instrument** — a deterministic mechanical tool that enumerates or verifies a
  ...
  measured-coverage layer the way `map/enumerate.mjs` does today — an
...
- **Injection-chain statics**: treating the LLM as an untrusted taint edge
  ...
  graph + `map/chains.mjs` already compute this from LLM-pass findings; a
