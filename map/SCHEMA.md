...
│   ├── view-descriptors.yaml       # GENERATED by descriptors.mjs --write (compile-package runs it): the register read (yardstick/README.md); validate.mjs recomputes every status and fails on drift
...
├── MAINTAINER-REPORT.md            # Pass 9 — THE LEAD human deliverable (views/improve/report.mjs)
...
tools: `views/improve/report.mjs` prepends it to `MAINTAINER-REPORT.md` (a downstream
...
```yaml
# view-security-gate.yaml — computed by the security view, read by views/improve/report.mjs
exposures:
