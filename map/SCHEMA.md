...
references this file rather than embedding the schema, and `map/validate.mjs`
...
off the number" is never reliable. The field is the single source of truth; the validator
...
renumber would rewrite the record for cosmetic reasons only. The validator checks just that
every id is a unique, well-formed `F-###` — never that it falls inside a per-dimension range.
...
Hand-authored map docs (`terrain.md`, `AI-NATIVE-EVAL.md`, `native/*.md`,
...
handoff file with `type: doc` + **`confidential: true`** — honest (they carry
the target's full findings and evidence paths), which is what lets
`START-HERE.md`'s intra-package links stay **relative** so the handoff works
...
**No deployment-stage scale.** A run does not carry a headline `gate:` or a
...
- **depth** — one authored, judged sentence: how good the *best instance* is, with
  finding ids. Depth can be high while coverage is low; keeping the two separate
  stops one excellent instance from inflating a whole dimension's score.
...
target), this roughly doubles fact-level repeatability and brings the dimensions that
vary most under a base sweep close to full agreement.
...
confidential: false         # RUN-LEVEL: true marks every compiled artifact (frontmatter
                            #   and footers) confidential. Never an engine default — a
                            #   deployment holding client runs turns it on here or via
                            #   --confidential on any compiler.
exec_summary:               # the overview: five authored paragraphs (scale, strength,
  scale: >                  # watch, maturity, gate), rendered in that order as plain prose,
    …                       # followed by the computed snapshot-stats table (finding/
  strength: >               # strength/gap/fact counts, by dimension). No editorialized
    …                       # verdict line — write scale/maturity/gate as flowing
  watch: >                  # paragraphs in plain language; strength/watch can be a
    …                       # sentence or two, since nothing else renders beside them.
  maturity: >               # Full maturity table with notes = body §1.
    …
  gate: >
    …
operating: prod             # OPTIONAL — not read by the compiler; safe to omit.
...
`views/improve/handoff.mjs` enforces two of these fail-closed: a `roadmap[].findings` id absent
...
no working gate) — the population the report's "what it can do" section counts as
unsupervised. Each one must trace to a remediation or a logged reason, or `validate.mjs`
fails closed (the remediation-side analogue of §6d's fail-closed discovery). Two authored
fields close the gap:
...
### 6d. Attack chains — computed, not authored (`map/chains.mjs`)

The report's lead risk section (**"The main risks, start to finish"**) is **computed from
the findings graph**, nothing authored. `map/chains.mjs` walks it:
...
## 7. What `map/validate.mjs` enforces

Run: `node assay.mjs validate <run-dir>` (exits non-zero on any
...
**Companion — `map/enumerate.mjs`:** the determinism scanner (README). It closes the
