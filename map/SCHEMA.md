...
│   ├── improve-leverage.md         # Improve view — faster/better opportunities (was view-leverage.md)
│   ├── improve-maturity.md         # Improve view — capability ladder (was view-maturity.md)
│   ├── improve-security.md         # Improve view — ALWAYS-ON; posture + gate (was view-security.md)
│   ├── improve-security-gate.yaml  # §6 — the security view's machine-readable exposures tail (was view-security-gate.yaml)
│   ├── maturity-inputs.yaml        # §6b — authored depth / censuses / earned flags
│   ├── censuses.md                 # §6b — census appendix: per-census method + item list
│   ├── improve-maturity-grades.yaml# §6b — GENERATED coverage (views/improve/maturity.mjs --write; was view-maturity-grades.yaml)
│   ├── improve-axes.md             # Pass 8.5 — the per-axis WALK (views/improve/axes.mjs; was view-axes.md)
│   ├── decisions.yaml              # OPTIONAL owner-triage overlay (map/decisions.mjs); absent = raw base
│   ├── yardstick.yaml              # GENERATED by yardstick/measure.mjs --write (views/compile.mjs runs it): the yardstick's measurement (yardstick/README.md); validate.mjs recomputes every status and fails on drift (was view-descriptors.yaml)
...
├── IMPROVE.md                      # Pass 9 — THE LEAD human deliverable (views/improve/report.mjs; was MAINTAINER-REPORT.md)
...
(`00-terrain.md`, `AI-NATIVE-EVAL.md`, `improve-*.md`, `censuses.md`,
`candidate-insights.md`) are authored with it. The generated docs get it from their
tools: `views/improve/report.mjs` prepends it to `IMPROVE.md` (a downstream
renderer, where one exists, reads from the first `## `, so it never reaches this
frontmatter), and `views/improve/handoff.mjs` writes
...
### 6a. `improve-security-gate.yaml` — the security view's exposures tail (was `view-security-gate.yaml`)

A reader falls back to the legacy `view-security-gate.yaml` name when the current one
is absent (`lib/legacy-name.mjs`), so a frozen run stays valid; a writer writes only
the current name.
...
### 6b. Maturity coverage — `maturity-inputs.yaml` (authored) → `improve-maturity-grades.yaml` (generated, was `view-maturity-grades.yaml`)
...
7. Every `F-###` cited in a view (`improve-*.md`), the gate sidecar, and
   ...
   citation-integrity check (a class of bug once caught by hand in a real run). A
   reader falls back to the legacy names (`view-*.md`, `MAINTAINER-REPORT.md`) when
   the current ones are absent (`lib/legacy-name.mjs`).
