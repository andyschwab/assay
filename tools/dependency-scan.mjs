...
// (registry/descriptors.yaml d-dependencies-known-clean — "no known-critical
// dependency vulnerability is present".) Its rows come in through
// tools/ingest.mjs (profile `dependency-scan`) and land on the shared
...
//
// Usage:
//   node tools/dependency-scan.mjs <target-dir> --out <file.json> [--timeout <seconds per lockfile, default 300>]
...
if (isMain(import.meta.url)) {
  ...
  if (!target || !out || !Number.isFinite(timeout) || timeout <= 0) {
    console.error('usage: node tools/dependency-scan.mjs <target-dir> --out <file.json> [--timeout <seconds per lockfile, default 300>]');
    process.exit(2);
  }
  try {
