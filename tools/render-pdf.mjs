#!/usr/bin/env node
// assay maintainer report → professional PDF.
...
import { WHO_LABEL, DIM_LABEL, GROUP_LABEL, channelLabel, humanizeToken } from './display.mjs';
...
function mastheadHtml() {
  ...
  return `<div class="masthead">
    <div class="mh-id"><div class="mh-kicker">assay · repository evaluation</div>` +
...
function securityRiskCards() {
  ...
  if (dispo.length) {
    ...
    for (const d of dispo) out.push(`<div class="blocker" style="border-left-color:var(--pending)">
      <div class="blocker-head"><span class="blocker-name">${esc(channelLabel(d.channel, prose.channel_notes || {}))}</span><span class="blocker-ids">${esc(d.reason)}</span></div>
...
function supervisionBar() {
  ...
  const s = buildSupervision(findings, prose.roadmap || [], prose.channel_notes || {});
...
function postureRows() {
  ...
  const ch = buildChains(findings, prose.channel_notes || {});
...
const withColophon = rebuilt.replace(/<p>(Generated by assay[\s\S]*?)<\/p>\s*(<\/section>)?\s*$/,
  '$2<section class="endplate"><div class="end-kicker">assay · repository evaluation</div><p class="colophon">$1</p></section>');
...
const cover = coverEnabled ? `<section class="cover">
  <div class="cover-kicker">assay · repository evaluation</div>
...
// the running header carries the confidentiality mark only when the run does
const CONFIDENTIAL = process.argv.includes('--confidential') || prose.confidential === true;
const head = `<div style="font-family:'Liberation Sans',sans-serif;font-size:7pt;color:#b7c0c4;width:100%;padding:0 0.7in;text-align:right;">${CONFIDENTIAL ? 'Confidential' : ''}</div>`;
...
const tmp = mkdtempSync(join(tmpdir(), 'assay-pdf-'));
