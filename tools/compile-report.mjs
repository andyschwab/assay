#!/usr/bin/env node
// repo-eval maintainer-report compiler.
...
import { DIM_LABEL, WHO_LABEL, humanizeToken, CHANNEL_LABEL } from './display.mjs';
...
function securityRisks() {
  ...
  if (dispo.length) {
    ...
    for (const d of dispo)
      out.push(`- **${cell(CHANNEL_LABEL[d.channel] || humanizeToken(d.channel))}** (_${cell(d.reason)}_) — ${cell(d.note)}`);
    out.push('');
  }
  return out.join('\n');
}
...
function chainUnknowns() {
  const { unresolved } = buildChains(findings);
...
function colophon() {
  return 'Generated by the AI-Native Framework repo-eval skill. A neutral evidence base is ' +
    'compiled into leverage, maturity, and security views. Severity and priority are computed ' +
    'from the evidence, not asserted, and the report issues no deploy/no-deploy verdict — it ' +
    'presents properties and risks and leaves the decision to the owner. The full evaluation ' +
    'artifacts accompany this report. Confidential.';
}
...
// OKF frontmatter so the generated report passes `npm run check` as a bundle
// file. `confidential: true` because the report carries the target's findings +
// evidence paths (the floor under instances/; harmless in a portable deployment).
// render-pdf.mjs renders from the first `## ` heading, so this never reaches the PDF.
